...
const Cart = () => {
  ...
  const calcularTotal = () => {
    let totalPizza = 0;
    cart.forEach((pizza) => {
      totalPizza += pizza.price * pizza.count; // Asegúrate de que pizza tiene la propiedad count
    });
    setTotal(totalPizza);
  };
  ...
  const handleIncrement = (id) => {
    const updatedCart = cart.map((pizza) => {
      if (pizza.id === id) {
        return { ...pizza, count: pizza.count + 1 }; // Aumenta la cantidad de pizza correspondiente
      }
  ...
  const handleDecrement = (id) => {
    const updatedCart = cart.map((pizza) => {
      if (pizza.id === id) {
        ...
        if (newCount <= 0) {
          return null; // Eliminar pizza si la cantidad es 0
        }
